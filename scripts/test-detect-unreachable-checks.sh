...
# $1 = case name, $2 = gh mock body, $3 = optional PR_LIMIT override (default 50).
# Populates $out/$code/$gh_output (the GITHUB_OUTPUT file's contents, for the
# generic-unreachable-prs handoff).
run_case() {
  local name="$1" gh_body="$2" limit="${3:-50}" out_file
  # shellcheck disable=SC2317  # invoked indirectly via exported function
  gh() { eval "$gh_body"; }
  export -f gh
  export gh_body
  out_file=$(mktemp)
  out=$(GH_REPO=SuxOS/example GRACE_MINUTES=30 PR_LIMIT="$limit" GITHUB_OUTPUT="$out_file" bash "$CHECK_SH" 2>&1)
  code=$?
...
echo "[1/16] gh pr list fails -> fail OPEN (no findings, exit 0)"
...
echo "[2/16] no open PRs -> exit 0, nothing to report"
run_case "no-prs" '
  case "$1" in
    pr) echo "[]" ;;
  esac'
...
echo "[3/16] rulesets unreadable for base -> skip PR, fail OPEN"
...
echo "[4/16] no required contexts on base -> skip PR, exit 0"
...
echo "[5/16] head commit is inside the grace window -> settle-gate skips it (never reports a fresh PR as jammed)"
...
echo "[6/16] checks still in flight on head -> not settled, skip"
...
echo "[7/16] all required contexts reported (exact + prefix-drift) -> reachable, exit 0"
run_case "reachable" "
  case \"\$1\" in
    pr) echo '[{\"number\":1,\"baseRefName\":\"main\",\"headRefOid\":\"abc123\"}]' ;;
    api) case \"\$2\" in
...
echo "[8/16] required context never reported, no disabled workflow found -> real jam (exit 1, generic remedy)"
...
echo "[9/16] required context's workflow is disabled_manually -> real jam (exit 1, disabled-workflow remedy)"
...
echo "[10/16] required context's workflow is disabled_inactivity -> real jam (exit 1, disabled-workflow remedy) (#610)"
run_case "never-reporting-disabled-inactivity" "
  case \"\$1\" in
    pr) echo '[{\"number\":10,\"baseRefName\":\"main\",\"headRefOid\":\"abc123\"}]' ;;
    api) case \"\$2\" in
           repos/*/rules/branches/*) echo '[{\"type\":\"required_status_checks\",\"parameters\":{\"required_status_checks\":[{\"context\":\"pin-consistency\"}]}}]' ;;
           repos/*/commits/*/check-runs) echo '{\"check_runs\":[]}' ;;
           repos/*/commits/*/status) echo '{\"statuses\":[]}' ;;
           repos/*/actions/workflows) echo '{\"workflows\":[{\"name\":\"pin-consistency\",\"state\":\"disabled_inactivity\"}]}' ;;
           repos/*/commits/*) echo '{\"commit\":{\"committer\":{\"date\":\"$OLD_DATE\"}}}' ;;
         esac ;;
  esac"
if [ "$code" -eq 1 ] && printf '%s' "$out" | grep -q 'auto-disabled for inactivity'; then
  note "a workflow auto-disabled for inactivity is classified as disabled, not generic (#610)"
else
  bad "expected exit 1 + 'auto-disabled for inactivity' remedy, got exit=$code out=$out"
fi
if [ "$gh_output" = "generic-unreachable-prs=" ]; then
  note "a pure-disabled_inactivity PR is excluded from generic-unreachable-prs, same as disabled_manually"
else
  bad "expected GITHUB_OUTPUT 'generic-unreachable-prs=' (empty), got: $gh_output"
fi

echo "[11/16] one gate disabled + one gate generic on the SAME PR -> excluded from generic-unreachable-prs (mixed cause)"
...
echo "[12/16] two PRs, only one pure-generic -> only that one is handed off"
...
echo "[13/16] two flagged PRs in one run -> actions/workflows fetched once, not re-fetched per PR (#386)"
...
echo "[14/16] required context is a longer '<workflow> / <job>' string, disabled workflow name is the shorter prefix -> classified as disabled (#380)"
...
echo "[15/16] actions/workflows fetch fails on a flagged PR -> unknown, not cached as zero disabled workflows (#518)"
run_case "workflows-fetch-fails" "
  case \"\$1\" in
    pr) echo '[{\"number\":21,\"baseRefName\":\"main\",\"headRefOid\":\"abc123\"}]' ;;
    api) case \"\$2\" in
           repos/*/rules/branches/*) echo '[{\"type\":\"required_status_checks\",\"parameters\":{\"required_status_checks\":[{\"context\":\"pin-consistency\"}]}}]' ;;
           repos/*/commits/*/check-runs) echo '{\"check_runs\":[]}' ;;
           repos/*/commits/*/status) echo '{\"statuses\":[]}' ;;
           repos/*/actions/workflows) return 1 ;;
           repos/*/commits/*) echo '{\"commit\":{\"committer\":{\"date\":\"$OLD_DATE\"}}}' ;;
         esac ;;
  esac"
if [ "$code" -eq 1 ] && printf '%s' "$out" | grep -q 'skipping disabled-workflow classification'; then
  note "a failed workflows-list fetch is surfaced as unknown, not silently treated as zero disabled workflows"
else
  bad "expected exit 1 + unknown-classification warning, got exit=$code out=$out"
fi
if printf '%s' "$out" | grep -q 'manually disabled'; then
  bad "a failed fetch must never be misreported as a confirmed disabled workflow"
else
  note "no false 'manually disabled' claim when the workflow list couldn't be read"
fi
if [ "$gh_output" = "generic-unreachable-prs=" ]; then
  note "a PR with unknown disabled-workflow status is excluded from generic-unreachable-prs, not assumed pure-generic"
else
  bad "expected GITHUB_OUTPUT 'generic-unreachable-prs=' (empty) for unknown status, got: $gh_output"
fi

echo "[16/16] cap-hit warning compares against the RAW pre-draft-filter count, not the post-filter count (#539)"
run_case "cap-hit-pre-draft-filter" '
  case "$1" in
    pr) echo "[{\"number\":1,\"baseRefName\":\"main\",\"headRefOid\":\"abc\",\"isDraft\":false},{\"number\":2,\"baseRefName\":\"main\",\"headRefOid\":\"def\",\"isDraft\":true},{\"number\":3,\"baseRefName\":\"main\",\"headRefOid\":\"ghi\",\"isDraft\":false}]" ;;
    api) case "$2" in
           repos/*/rules/branches/*) return 1 ;;
         esac ;;
  esac' 3
if [ "$code" -eq 0 ] && printf '%s' "$out" | grep -q '::warning::open PR count hit pr-limit (3)'; then
  note "the pr-limit warning still fires on a truncated raw page even though one of the 3 raw PRs is a draft"
else
  bad "expected the pr-limit warning to fire on the raw count despite draft filtering, got exit=$code out=$out"
fi
if printf '%s' "$out" | grep -q 'open non-draft PRs to check: 2'; then
  note "the 'to check' count is still the post-filter count (draft stripped out before the per-PR loop)"
else
  bad "expected post-filter count of 2 (one of 3 stripped as draft), got out=$out"
fi
